...
def _auto_task_wrapper(module_name, class_name, func_name, *args, **kwargs):
    try:
        ...
        instance = class_(
            auto_task_service=auto_task_executor_service
        )  # instantiate class using _AutoTaskExecutorService
        ...
        if executor_func_name:
            ...
            logger.error(
                "Trying to invoke _auto_task_wrapper for unregistered task "
                "with module: %s class: %s func: %s args: %s and kwargs: %s",
                module_name,
                class_name,
                func_name,
                args,
        ...
        try:
            ...
            if exc.max_retries_func:
                ...
                logger.error(
                    "Reached max retries in auto task {}.{}.{} with error: {}".format(
                        module_name, class_name, func_name, repr(exc)
                    )
                )
...
class AutoTaskService(object):
    def register_task(
            self, method: typing.Any, queue_name: str = None, max_retries: int = None
    ):
...
class _AutoTaskExecutorService(AutoTaskService):
    ...
    def register_task(self, method, queue_name: str = None, max_retries: int = None):
        ...
        super(_AutoTaskExecutorService, self).register_task(
            method, queue_name, max_retries
        )
