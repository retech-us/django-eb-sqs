...
class WorkerService(object):
    ...
    def process_queues(self, queue_names: typing.List[str]):
        ...
        logger.debug(
            "[django-eb-sqs] Connecting to SQS: {}".format(", ".join(queue_names))
        )
        ...
        logger.debug(
            "[django-eb-sqs] Connected to SQS: {}".format(", ".join(queue_names))
        )
        ...
        logger.info(
            "[django-eb-sqs] NO_QUEUES_WAIT_TIME_S = {}".format(
                settings.NO_QUEUES_WAIT_TIME_S
            )
        )
        logger.info(
            "[django-eb-sqs] MAX_NUMBER_OF_MESSAGES = {}".format(
                settings.MAX_NUMBER_OF_MESSAGES
            )
        )
        logger.info(
            "[django-eb-sqs] AUTO_ADD_QUEUE = {}".format(settings.AUTO_ADD_QUEUE)
        )
        ...
        logger.info(
            "[django-eb-sqs] DEFAULT_MAX_RETRIES = {}".format(
                settings.DEFAULT_MAX_RETRIES
            )
        )
        logger.info(
            "[django-eb-sqs] REFRESH_PREFIX_QUEUES_S = {}".format(
        ...
        while not self._exit_gracefully:
            if (
                ...
                logger.debug(
                    "[django-eb-sqs] Updated SQS queues: {}".format(
        ...
        for queue in queues:
            ...
            try:
                ...
                if (
                    ...
                    logger.warning(
                        "[django-eb-sqs] Error polling queue {}: {}".format(
                            queue.url, exc
                        ),
                        exc_info=True,
                    )
                ...
                logger.warning(
                    "[django-eb-sqs] Error polling queue {}: {}".format(queue.url, exc),
                    exc_info=True,
                )
    ...
    def delete_messages(self, queue, msg_entries: list):
        if len(msg_entries) > 0:
            ...
            if num_failed > 0:
                logger.warning(
                    "[django-eb-sqs] Failed deleting {} messages: {}".format(
                        num_failed, failed
                    )
                )
    ...
    def _process_message(self, msg, worker):
        ...
        try:
            ...
            logger.warning(
                "[django-eb-sqs] Handling message {} got error: {}".format(
