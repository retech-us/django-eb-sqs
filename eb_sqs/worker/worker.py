...
class Worker(object):
    ...
    def delay(self, group_id: typing.Optional[str], queue_name: str, func: callable, args: typing.Iterable,
              kwargs: dict, max_retries: int,
              use_pickle: bool, delay: int, execute_inline: bool):
        worker_task = WorkerTask(str(uuid.uuid4()), group_id, queue_name, func, args, kwargs,
                                 max_retries, 0, None,
                                 use_pickle)
        return self._enqueue_task(worker_task, delay, execute_inline, False, True)

    def retry(self, worker_task: WorkerTask, delay: int, execute_inline: bool, count_retries: bool):
    ...
    @classmethod
    def _execute_task(cls, worker_task: WorkerTask):
