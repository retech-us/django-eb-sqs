...
def _get_kwarg_val(kwargs: dict, key: str, default: typing.Any) -> typing.Any:
    return kwargs.pop(key, default) if kwargs else default


def func_delay_decorator(
    func: callable, queue_name: str, max_retries_count: int, use_pickle: bool
):
    def wrapper(*args, **kwargs) -> typing.Any:
        ...
        pickle = _get_kwarg_val(
            kwargs, "use_pickle", use_pickle if use_pickle else settings.USE_PICKLE
        )
...
def func_retry_decorator(worker_task: WorkerTask):
...
class task(object):
    def __init__(
        self, queue_name: str = None, max_retries: int = None, use_pickle: bool = None
    ):
    ...
    def __call__(self, *args, **kwargs):
        func = args[0]
        func.retry_num = 0
        func.delay = func_delay_decorator(
            func, self.queue_name, self.max_retries, self.use_pickle
        )
        return func
